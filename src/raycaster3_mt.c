...
void			*draw_textured_floor_and_ceiling_mt(void *_q)
{
	t_draw_tex_floor_ceiling	*q = (t_draw_tex_floor_ceiling *)_q;
	while (q->ctr.y < q->upper)
	{
		q->cur_y = q->ctr.y - q->cub3d->mlx->resolution.y / 2;
		q->camera_y_rel = q->cur_y == 0.0 ? 0.0 : q->camera_y / q->cur_y;
		q->floor_step.x = q->camera_y_rel * (q->ray_dir_right.x - q->ray_dir_left.x)
			/ q->cub3d->mlx->resolution.x;
		q->floor_step.y = q->camera_y_rel * (q->ray_dir_right.y - q->ray_dir_left.y)
			/ q->cub3d->mlx->resolution.x;
		q->floor.x = q->cub3d->player->pos.x + q->camera_y_rel * q->ray_dir_left.x;
		q->floor.y = q->cub3d->player->pos.y + q->camera_y_rel * q->ray_dir_left.y;
		q->ctr.x = 0;
		draw_textured_floor_and_ceiling_norm1(q->cub3d->raycast, q->cub3d, q);
		q->ctr.y++;
	}
	return (NULL);
}
					...
					t_cub3d *cub3d)
{
	...
	t_draw_tex_floor_ceiling qa;
	t_draw_tex_floor_ceiling qb;
	t_draw_tex_floor_ceiling qc;
	...
	q.cub3d = cub3d;
	q.upper = cub3d->mlx->resolution.y;

	ft_memcpy(&qa, &q, sizeof(t_draw_tex_floor_ceiling));
	ft_memcpy(&qb, &q, sizeof(t_draw_tex_floor_ceiling));
	ft_memcpy(&qc, &q, sizeof(t_draw_tex_floor_ceiling));

	qb.ctr.y += q.ctr.y / 3;
	qc.ctr.y += 2 * (q.ctr.y / 3);
	qa.upper = qb.ctr.y;
	qb.upper = qc.ctr.y;
	qc.upper = cub3d->mlx->resolution.y;

	pthread_create(&qa.thread_id, NULL, draw_textured_floor_and_ceiling_mt, &qa);
	pthread_create(&qb.thread_id, NULL, draw_textured_floor_and_ceiling_mt, &qb);
	pthread_create(&qc.thread_id, NULL, draw_textured_floor_and_ceiling_mt, &qc);
	pthread_join(qa.thread_id, NULL);
	pthread_join(qb.thread_id, NULL);
	pthread_join(qc.thread_id, NULL);
}
